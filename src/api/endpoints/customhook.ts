...
// hook
export const useGetMovieLists = (param: string, page: number): UseQueryResult<Data, Error> => {
...
export const useGetMultiMovieLists = (): {
  ...
  const queries = useQueries({
    queries: filterCagegories?.map((movieCategory) => ({
      ...
      queryFn: (): Promise<ApiResponse> => getMovieLists(movieCategory.param, movieCategory.page),
      staleTime: 60 * 1000 * 60,
    })),
    combine: (results) => {
      return {
